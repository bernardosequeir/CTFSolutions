def biggest_mul_near_grid():
    grid = [[8, 2, 22, 97, 38, 15, 00, 40, 00,
             75, 4, 5, 7, 78, 52, 12, 50, 77, 91, 8],
            [49, 49, 99, 40, 17, 81, 18, 57, 60,
             87, 17, 40, 98, 43, 69, 48, 4, 56, 62, 00],
    ...
    while (y < len(grid)):
        while(x < len(grid)):
            high_mul = check_mul(grid, x, y, high_mul)
            x += 1
        y += 1
    return high_mul


def check_mul(grid, x, y, high_mul):
    mul = check_left(grid, x, y)
    if(mul > high_mul):
        high_mul = mul
    mul = check_up(grid, x, y)
    if(mul > high_mul):
        high_mul = mul
    mul = check_right(grid, x, y)
    if(mul > high_mul):
        high_mul = mul
    mul = check_down(grid, x, y)
    if(mul > high_mul):
        high_mul = mul
    mul = check_diag_lu(grid, x, y)
    if(mul > high_mul):
        high_mul = mul
    mul = check_diag_ld(grid, x, y)
    if(mul > high_mul):
        high_mul = mul
    mul = check_diag_rd(grid, x, y)
    if(mul > high_mul):
        high_mul = mul
    mul = check_diag_ru(grid, x, y)
    if(mul > high_mul):
        high_mul = mul
    return high_mul
...
def check_diag_ru(grid, x, y):
    xi = x+3
    yi = y-3
    mul = 1
    while(xi >= x and yi <= y):
        if(xi >= len(grid) or yi < 0):
            return 0
        mul *= grid[yi][xi]
        yi += 1
        xi -= 1
...
def check_diag_rd(grid, x, y):
    xi = x+3
    yi = y+3
    mul = 1
    while(xi >= x and yi >= y):
        if(xi >= len(grid) or yi >= len(grid)):
            return 0
        mul *= grid[yi][xi]
        yi -= 1
        xi -= 1
...
def check_diag_ld(grid, x, y):
    xi = x-3
    yi = y+3
    mul = 1
    while(xi <= x and yi >= y):
        if(xi < 0 or yi >= len(grid)):
            return 0
        mul *= grid[yi][xi]
        yi -= 1
        xi += 1
